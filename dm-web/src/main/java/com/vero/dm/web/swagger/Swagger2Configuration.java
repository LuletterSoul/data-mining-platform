 ...
 * @version 1.5 created in 20:32 2017/12/14.
 * @since easyclass
 */
...
public class Swagger2Configuration
{
    ...
    public Docket createRestApi()
    {
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select().apis(
            RequestHandlerSelectors.basePackage(SWAGGER_SCAN_BASE_PACKAGE)).paths(
    ...
    private ApiInfo apiInfo()
    {
        return new ApiInfoBuilder().title(
            "数据挖掘实践教学平台RESTful APIs说明文档").description(
                "本文档说明了每个接口的输入参数，及其输出的JSON数据形式。" + "此文档是前后端分离模式开发的标准，要求前端发送符合输入参数的HTTP请求，让每个请求携带着符合服务器要求的用户授权信息，经服务器授权通过后返回JSON数据流。"
                                                      ...
                                                      + "总而言之，在RESTful API万物皆资源的体系框架下，后端的实体也被抽象为资源，只是资源可能是单一的，也可以是被组合优化过的，也可能是跨越了多个资源联结统计出来的，对前端暴露出来的无非就是一个一个名词。前端通过动词的HTTP方法，得到名词化的JSON资源，仅此而已。").version(
                                                          "2.0").contact(
                                                              new Contact("刘祥德", "/",
                                                                  "luvletterU@njust.edu.cn")).build();
    }
}
