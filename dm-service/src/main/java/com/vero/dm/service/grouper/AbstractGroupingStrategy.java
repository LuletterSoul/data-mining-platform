...
public abstract class AbstractGroupingStrategy implements GroupingStrategy
{
                                        ...
                                        List<String> tasksIds)
    {
        ...
        Integer candidateSize = params.getSpecifiedDividingStudents().size();
        ...
        Iterator stuIt = candidates.iterator();
            ...
            while (stuIt.hasNext() && taskIt.hasNext())
            {
                ...
                if (i == gradient)
                {
                    ...
                    DataMiningGroup group = buildGroup(perGroupStudents, i, builderId, taskId);
                    // 生成预览分组情况，待管理员确认;
        ...
        {
            DataMiningGroup group = buildGroup(perGroupStudents, 1, params.getBuilderId(),
                    params.getTaskId());
            previewDefaultGroups.add(group);
        }
    ...
    protected abstract DataMiningGroup buildGroup(List<Student> perGroupStudents, int i,
                                                  String builderId, String taskId);
