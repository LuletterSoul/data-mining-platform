...
public class StudentDao extends BaseDao<Student, String>
{
    ...
    /**
     * 查询出所有对外暴露的DTO 注意测试时候Student Model 的内嵌对象应该使用外连接 避免查询出空列表
     * 
     * @return
     */
    ...
    public int deleteStudentById(String studentId)
    {
        ...
        return getSession().createQuery(hqlString).setParameter("studentId",
            studentId).executeUpdate();
    }
    ...
    public Student getStudentById(String studentId)
    {
        ...
        return (Student)getSession().createQuery(hqlString).setParameter("studentId",
            studentId).getSingleResult();
    }
    ...
    public int markStudents(List<String> studentIds)
    {
        ...
        return getSession().createNativeQuery(sqlString).setParameter("studentIds",
            studentIds).executeUpdate();
    }
    ...
    public int deleteByArray(List<String> studentIds)
    {
        ...
        return getSession().createQuery(hqlString).setParameter("studentIds",
            studentIds).executeUpdate();
    }
    ...
    public FavoriteStatus getFavoriteStatusPersisted(Integer statusId)
    {
        ...
        return (FavoriteStatus)getSession().createQuery(hqlString).setParameter("statusId",
            statusId).getSingleResult();
    }

    public StudentStatus getStudentStatus(Integer statusId)
    {
        String hqlString = "from StudentStatus s where s.statusId = :statusId";
        return (StudentStatus)getSession().createQuery(hqlString).setParameter("statusId",
            statusId).getSingleResult();
    }
