...
import com.vero.dm.repository.query.PaginationDescriptor;
...
public interface BaseRepository<E, PK extends Serializable>
{
    ...
    /**
     * 基于JPA2.0规范的Criteria新型查询,函数<code>createCriteria()</code> 已被建议弃用
     * 利用SessionFactory返回的Session<code>getCriteriaBuilder()</code>
     * 建立新型的<code>CriteriaQuery</></code> 利于构建良好的domain model的架构 执行更完整的type-safe标准查询
     * 
     * @return 当前Session下criteria query object {@link org.hibernate}
    ...
    /**
     * 与{@link Pageable}结合的分页查询机制 通用的分页查询
     * 
     * @param pageable
     *            分页的基础信息
     * @return 指定页面的数据
     ...
     * 根据hql语法批量更新
     * 
     * @param updateMappingValues
     *            传入更新属性参数对
     * @param conditionMappingValues
     *            出入条件属性参数对
     * @return 返回自定义的Map类型
     */
