...
public class GroupDao extends BaseDao<DataMiningGroup, String>
{
    ...
    public List<DataMiningGroup> fetchGroups(List<String> groupIds)
    {
        ...
        return getSession().createQuery(hqlString).setParameterList("groupIds",
            groupIds).getResultList();
    }

    public DataMiningGroup fetchGroup(String groupId)
    {
        String hqlString = "select distinct g from DataMiningGroup g "
                           + "left join g.groupMembers " + "left join DataMiningTask t "
                           + "left join t.algorithms " + "where g.groupId = :groupId";
        return (DataMiningGroup)getSession().createQuery(hqlString).setParameter("groupId",
            groupId).getSingleResult();
    }

    public List<DataMiningGroup> getGroups(List<String> groupIds)
    {
        String hqlString = "select g from DataMiningGroup g " + "where g.groupId in :groupIds";
        return getSession().createQuery(hqlString).setParameterList("groupIds",
            groupIds).getResultList();
    }
    ...
    public List<Student> fetchGroupMembers(String groupId)
    {
        ...
        return getSession().createQuery(hqlString).setParameter("groupId",
    ...
    public int removeMemberById(String groupId, String studentId)
    {
        ...
        return getSession().createNativeQuery(sqlString).setParameter("studentId",
            studentId).setParameter("groupId", groupId).executeUpdate();
    }
    ...
    public int removeMembersWithArray(String groupId, List<String> studentIds)
    {
        ...
        return getSession().createQuery(hqlString).setParameter("groupId", groupId).setParameter(
            "studentIds", studentIds).executeUpdate();
    }
