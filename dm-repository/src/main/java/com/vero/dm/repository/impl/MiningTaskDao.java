...
import com.vero.dm.repository.MiningTaskRepository;
...
public class MiningTaskDao extends BaseDao<DataMiningTask, String> implements MiningTaskRepository
{
    ...
    {
        String hqlString = "select t.groups from DataMiningTask t " + "left join fetch t.groups g "
                           + "left join fetch g.groupMembers m " + "where t.taskId = :taskId";
    ...
    public List<DataMiningGroup> fetchPartInvolvedGroups(String taskId, List<String> groupIds)
    {
        ...
        return getSession().createQuery(hqlString).setParameter("taskId", taskId).setParameter(
            "groupIds", groupIds).getResultList();
    }
    ...
    public int removeInvolvedGroups(String taskId, List<String> groupIds)
    {
        ...
        return getSession().createQuery(hqlString).setParameter("taskId", taskId).setParameter(
            "groupIds", groupIds).executeUpdate();
    }

    @Override
    public Algorithm getConfiguredAlgorithm(String taskId, String algorithmId)
    {
        String hqlString = "select a from DataMiningTask t " + "left join fetch t.algorithms a "
                           + "where t.taskId = :taskId and a.algorithmId = :algorithmId";
        return (Algorithm)getSession().createQuery(hqlString).setParameter("taskId",
            taskId).setParameter("algorithmId", algorithmId).getSingleResult();
    }
    ...
    public int removeAllInvolvedAlgorithm(String taskId)
    {
        ...
        return getSession().createQuery(hqlString).setParameter("taskId", taskId).executeUpdate();
    }

    @Override
    public int removePartInvolvedAlgorithms(String taskId, List<String> algorithmIds)
    {
        String hqlString = "delete DataMiningTask.algorithms a " + "where a.algorithmId "
                           + "in :algorithmIds and DataMiningTask.taskId =:taskId";
        return getSession().createQuery(hqlString).setParameter("algorithmIds",
            algorithmIds).setParameter("taskId", taskId).executeUpdate();
    }
    ...
    public int removeRefSets(String taskId, List<String> containerIds)
    {
        ...
        return getSession().createQuery(hqlString).setParameter("taskId", taskId).setParameter(
            "containerIds", containerIds).executeUpdate();
    }
    ...
    {
        String hqlString = "select c from DataMiningTask t " + "left join fetch  t.collections c";
        return getSession().createQuery(hqlString).setParameter("taskId", taskId).setParameter(
            "containerIds", containerIds).getResultList();
    }
